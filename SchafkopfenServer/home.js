...
socket.onmessage = message => { // Handle alle Messages, die vom Server kommen
    ...
    else if (state === "choosing" && message.data.startsWith("Was würdest du spielen?")) { // Spielart auswählen
        state = "selecting";
    ...
    else if (message.data.startsWith("Gespielt wird:")) { // Spielart einlesen
        createLog(message.data);
        // TODO hier einlesen was gespielt wird und danach ne Karte erstellen
    }
    ...
    if (message.data.contains("Spieler")) { // Nach dem Spiel aufräumen
        setTimeout(() => document.getElementById("spielart").children[0].remove(), removal_timeout);
    }
...
function clickHandler() {
    if (state === "selecting") {
        if (this.getAttribute("value") === "Ramsch") {
            send("Ramsch")
        } else if (this.getAttribute("value") === "Ober") {
            send("Geier")
        } else if (this.getAttribute("value") === "Unter") {
            send("Wenz")
        } else if (this.getAttribute("value") === "Ass") {
            send("Solo:" + this.getAttribute("color"))
        } else {
            send("Sauspiel:" + this.getAttribute("color"))
        }
        Array.from(document.getElementById("cards").children).forEach(card => card.remove()); // Ramsch Karte entfernen
